...
//! shell or the clock. The run rules never see it — the core only ever receives a
//! [`Loadout`], and this module is simply one way to build one. That keeps the
//! whole meta layer additive: it can grow without touching a single rule.
//!
//! A fresh player flies the base ship — always fully playable — and earns options
//! by playing: scores, cleared stages, wins, and wins at Ascension tiers.

use super::Loadout;

/// A ship option a player can unlock and fly with. Each folds into the run's
/// starting [`Loadout`].
...
pub enum Content {
    ...
    Pierce,
    /// Begin with an extra life.
    Reserve,
    /// Begin with the rapid cadence (weapon tier 3).
    Rapid,
    /// Begin with a half overdrive charged.
    Charged,
    /// Begin with side drones (weapon tier 4).
    Drones,
}

/// Every unlockable option — the order the collection screen shows them in, and
/// the order of their bits.
pub const ALL: [Content; 7] = [
    Content::Spread,
    Content::Shield,
    Content::Pierce,
    Content::Reserve,
...
impl Content {
    ...
    pub fn condition(self) -> &'static str {
        match self {
            ...
            Content::Shield => "CLEAR A STAGE",
            Content::Pierce => "SCORE 1500",
            Content::Reserve => "SCORE 3500",
            Content::Rapid => "WIN A SORTIE",
            Content::Charged => "WIN AT ASCENSION 1",
            Content::Drones => "WIN AT ASCENSION 2",
        }
    }
    ...
    fn earned_by(self, outcome: Outcome) -> bool {
        match self {
            ...
            Content::Shield => outcome.stages_cleared >= 1,
            ...
            Content::Drones => outcome.won && outcome.tier >= 2,
        }
    }
}
...
pub struct Outcome {
    /// The run won — a Sortie's final mothership fell.
    pub won: bool,
    /// How many stages it cleared.
...
/// The options a player has unlocked, as a small bitset. Empty is a fresh player,
/// who flies the base ship.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
...
impl Unlocked {
    ...
    pub fn loadout(self) -> Loadout {
        ...
        if self.has(Content::Drones) {
            loadout.start_weapon = loadout.start_weapon.max(4);
        }
...
mod tests {
    ...
    fn unlocked_options_build_the_loadout() {
        ...
        assert_eq!(loadout.bonus_lives, 1, "reserve grants a life");
        assert!(loadout.start_overdrive, "charged primes the overdrive");
    }
    ...
    #[test]
    fn clearing_a_stage_earns_the_shield() {
        let mut unlocked = Unlocked::default();
        let newly = unlocked.record(Outcome {
            stages_cleared: 1,
            ..Default::default()
